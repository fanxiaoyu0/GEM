...
def get_smiles_list_pkl():
    dataset_df = pd.read_csv("../data/raw/dataset.csv")
    smiles_list = dataset_df["smiles"].tolist()
    smiles_list = list(set(smiles_list))
    print(len(smiles_list))
    pkl.dump(smiles_list, open('../data/intermediate/smiles_list.pkl', 'wb'))
...
def calculate_3D_structure():
    smiles_list_unique = pkl.load(open('../data/intermediate/smiles_list.pkl', 'rb'))
    global smiles_to_graph_dict
    ...
    for t in threads:
        t.join()
    pkl.dump(smiles_to_graph_dict, open('../data/intermediate/smiles_to_graph_dict.pkl', 'wb'))
    print('Valid smiles count:', len(smiles_to_graph_dict))

def construct_data_list():
    dataset_df = pd.read_csv("../data/raw/dataset.csv")
    smiles_to_graph_dict:dict = pkl.load(open('../data/intermediate/smiles_to_graph_dict.pkl','rb'))
    ...
    pkl.dump(data_list_train, open('../data/intermediate/data_list_train.pkl', 'wb'))
    pkl.dump(data_list_validate, open('../data/intermediate/data_list_validate.pkl', 'wb'))
    pkl.dump(data_list_test, open('../data/intermediate/data_list_test.pkl', 'wb'))
...
def get_data_loader(batch_size):
    data_list_train = pkl.load(open('../data/intermediate/data_list_train.pkl', 'rb'))
    data_list_validate = pkl.load(open('../data/intermediate/data_list_validate.pkl', 'rb'))
    data_list_test = pkl.load(open('../data/intermediate/data_list_test.pkl', 'rb'))
...
def test(model_version):
    ...
    model = ADMET()
    model.set_state_dict(pdl.load("../weight/" + model_version + ".pkl"))
    model.eval()
